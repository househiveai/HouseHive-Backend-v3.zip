...
# =============================
# APP + CORS (order matters)
# =============================
app = FastAPI(title="HouseHive Backend", version="3.0.0")

app.add_middleware(
    CORSMiddleware,
    allow_origins=CORS_ORIGINS if any(CORS_ORIGINS) else ["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
...
def create_tenant(payload: TenantCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    t = Tenant(
        ...
        property_id=payload.property_id, owner_email=user.email
    )
    db.add(t)
    db.commit()
    db.refresh(t)
...
def get_insights(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if pc == 0:
        return {"summary": "No data yet. Add your first property to get started!",
                "property_count": 0, "tenant_count": 0, "open_tasks": 0, "reminders": 0}
